...
int abSearch(int depth, int alpha, int beta, int role)
{
    ...
    if (competitor_score >= 100000)
    {
        return MIN + 1000 + (DEPTH - depth);
    }
    if (depth == 0)
    {
        recordHashItem(depth, my_score - competitor_score, EXACT);
...
// If this is the third step(with 2 black ), where you can use the swap rule, your output could be either (-1, -1) to indicate that you choose a swap, or a position (x,y) as normal.
...
std::pair<int, int> action(std::pair<int, int> loc)
{
    ...
    if (x == -1 && y == -1)
    {
        if (turn == 1)
        {
            ...
            board[2][7] = ai_side;
            update_score(position(2, 7), ai_side);
            next_point = position(2, 7);
            ppm.add(board, position(2, 7));
